...
class DiscourseLoader(BaseLoader):
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if not config:
            raise ValueError(
                "DiscourseLoader requires a config. Check the documentation for the correct format - `https://docs.ai/data-sources/discourse`"  # noqa: E501
            )

        self.domain = config.get("domain")
        if not self.domain:
            raise ValueError(
                "DiscourseLoader requires a domain. Check the documentation for the correct format - `https://docs.ai/data-sources/discourse`"  # noqa: E501
            )

    def _check_query(self, query):
        if not query or not isinstance(query, str):
            raise ValueError(
                "DiscourseLoader requires a query. Check the documentation for the correct format - `https://docs.ai/data-sources/discourse`"  # noqa: E501
            )
    ...
    def load_data(self, query):
        ...
        response_data = {"file_id": doc_id, "data": data}

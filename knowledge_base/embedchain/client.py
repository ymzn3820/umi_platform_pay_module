...
from knowledge_base.embedchain.constants import CONFIG_DIR, CONFIG_FILE


class Client:
    def __init__(self, api_key=None, host="https://apiv2.ai"):
        ...
        if api_key:
            if self.check(api_key):
                ...
                raise ValueError(
                    "Invalid API key provided. You can find your API key on https://app.ai/settings/keys."
                )
        else:
            if "api_key" in self.config_data:
                ...
                raise ValueError(
                    "You are not logged in. Please obtain an API key from https://app.ai/settings/keys/"
                )

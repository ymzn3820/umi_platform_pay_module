...
def create(template, extra_args):
    ...
    with open("json", "w") as file:
        ...
        console.print(
            f"🎉 [green]All done! Successfully created `json` with '{template}' as provider.[/green]"
        )
...
def deploy_streamlit():
    ...
    try:
        ...
        For more information, please refer to https://docs.ai/deployment/streamlit_io
        [/bold yellow]
                      \n\n"""
        )
...
def deploy_hf_spaces(ec_app_name):
    if not ec_app_name:
        console.print("❌ [bold red]'name' not found in json[/bold red]")
        return

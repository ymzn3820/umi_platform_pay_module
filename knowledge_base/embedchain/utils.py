...
def use_pysqlite3():
    ...
    if platform.system() == "Linux" and sqlite3.sqlite_version_info < (3, 35, 0):
        try:
            ...
            #     [sys.executable, "-m", "pip", "install", "pysqlite3_binary-0.5.2-cp311-cp311-manylinux_2_24_x86_64.whl", "--quiet", "--disable-pip-version-check"]
            # )
...
def detect_datatype(source: Any) -> DataType:
    ...
    if url:
        from langchain_community.document_loaders.youtube import ALLOWED_NETLOCK as YOUTUBE_ALLOWED_NETLOCS
...
def is_valid_json_string(source: str):
    try:
        ...
        logging.error(
            ...
            Check the docs to see the supported formats - `https://docs.ai/data-sources/json`"
        )
        return False

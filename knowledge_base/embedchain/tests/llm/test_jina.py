...
def test_get_llm_model_answer(config, mocker):
    mocked_get_answer = mocker.patch("llm.jina.JinaLlm._get_answer", return_value="Test answer")
...
def test_get_llm_model_answer_with_streaming(config, mocker):
    config.stream = True
    mocked_jinachat = mocker.patch("llm.jina.JinaChat")

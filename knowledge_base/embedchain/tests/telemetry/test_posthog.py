...
from telemetry.posthog import AnonymousTelemetry
...
class TestAnonymousTelemetry:
    def test_init(self, mocker):
        ...
        mock_posthog = mocker.patch("telemetry.posthog.Posthog")
    ...
    def test_init_with_disabled_telemetry(self, mocker, monkeypatch):
        mocker.patch("telemetry.posthog.Posthog")
        telemetry = AnonymousTelemetry()
        assert telemetry.enabled is False
        assert telemetry.posthog.disabled is True

    def test_get_user_id(self, mocker, tmpdir):
        mock_uuid = mocker.patch("telemetry.posthog.uuid.uuid4")
        mock_uuid.return_value = "unique_user_id"
        config_file = tmpdir.join("config.json")
        mocker.patch("telemetry.posthog.CONFIG_FILE", str(config_file))
        telemetry = AnonymousTelemetry()

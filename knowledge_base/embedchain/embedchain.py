...
from knowledge_base.embedchain.config import AddConfig, BaseLlmConfig, ChunkerConfig
from knowledge_base.embedchain.config.apps.base_app_config import BaseAppConfig
...
from knowledge_base.embedchain.models.data_type import (DataType, DirectDataType,
                                         IndirectDataType, SpecialDataType)
from knowledge_base.embedchain.utils import detect_datatype, is_valid_json_string
...
class EmbedChain(JSONSerializable):
        ...
        **kwargs: Optional[Dict[str, Any]],
        ...
        if data_type:
            try:
                ...
                logging.info(
                    f"Invalid data_type: '{data_type}', using `custom` instead.\n Check docs to pass the valid data type: `https://docs.ai/data-sources/overview`"  # noqa: E501
                )
    ...
    def _get_existing_doc_id(self, chunker: BaseChunker, src: Any):
        ...
        if chunker.data_type.value in [item.value for item in DirectDataType]:
            ...
            if len(existing_embeddings.get("metadatas", [])) > 0:
                return existing_embeddings["metadatas"][0]["file_id"]
            else:
                return None
    ...
    def _load_and_embed(
        ...
        **kwargs: Optional[Dict[str, Any]],
        ...
        if existing_doc_id and existing_doc_id != new_doc_id:
            ...
            try:
                self.db.delete({"file_id": existing_doc_id})
            except Exception:
                print('Not find such file, pass')
    ...
    def _retrieve_from_database(
        ...
        **kwargs: Optional[Dict[str, Any]],
        ...
        logging.info("where condition is {}".format(where))
    ...
    def query(
        ...
        **kwargs: Dict[str, Any],
        ...
        contexts = self._retrieve_from_database(
            input_query=input_query, config=config, where=where, **kwargs
        )
    ...
    def chat(
        ...
        **kwargs: Dict[str, Any],
        ...
        # add conversation in memory, currently do not need

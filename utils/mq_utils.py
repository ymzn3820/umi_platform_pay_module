...
class RabbitMqObj:
    ...
    def set_delay(self, delay):
        ...
        if delay:
            ...
            if delay_queue:
                arguments["x-dead-letter-routing-key"] = delay_queue
            if timeout:
...
class RabbitMqConsumer(RabbitMqObj):
    ...
    def bin_func(self, data):
        ...
        result = self.channel.queue_declare(
            queue=queue, durable=self.queue_durable, arguments=arguments
        )
        # 告诉rabbitmq，用callback来接收消息
        self.channel.basic_consume(
            result.method.queue, callback, auto_ack=self.auto_ack
        )
    ...
    def handel_error(self, fun):
        ...
        def wrapper(self, ch, method, properties, body):
            try:
                ...
                logger.error(
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M')}rabbit : {str(e)} , {str(traceback.format_exc())} body:{body}"
                )
                ch.basic_reject(
    ...
    def retry_err(self, fun):
        ...
        def wrapper(self, ch, method, properties, body):
            try:
                ...
                logger.error(
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M')}rabbit : {str(e)} body:{str(body)}"
                )
                ch.basic_reject(
                    delivery_tag=method.delivery_tag, requeue=True
                )  # 表示拒绝的消息将重新排队等待重新投递给其他消费者
